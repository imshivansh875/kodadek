...
export const NowPlayingContextProvider: ParentComponent = (props) => {
  ...
  const { authToken } = useSpotifyAuth();
  ...
  const setPause = async () => {
    const response = await setSpotifyPause(authToken());
    setTimeout(() => nowPlayingQuery.refetch(), 100);
  };

  const setPlay = async () => {
    const response = await setSpotifyPlay(authToken());
    setTimeout(() => nowPlayingQuery.refetch(), 100);
  };

  const setPrevious = async () => {
    const response = await setSpotifyPrevious(authToken());
    setTimeout(() => nowPlayingQuery.refetch(), 100);
  }

  const setNext = async () => {
    const response = await setSpotifyNext(authToken());
    setTimeout(() => nowPlayingQuery.refetch(), 100);
  }

  const setShuffle = async () => {
    const response = await setSpotifyShuffle(authToken(), !state.nowPlaying?.shuffle_state);
    setTimeout(() => nowPlayingQuery.refetch(), 100);
  }

  const getSaved = async (id: string) => {
    const response = await getSpotifySaved(authToken(), id);
